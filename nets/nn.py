...
class DarkNet(torch.nn.Module):
    def __init__(self, width, depth, csp):
        ...
        # p1/2
        self.p1.append(Conv(width[0], width[1], torch.nn.SiLU(), k=3, s=2, p=1)) #(B, 3, 640, 640) -> (B, 16, 320, 320)
        # p2/4
        self.p2.append(Conv(width[1], width[2], torch.nn.SiLU(), k=3, s=2, p=1)) #(B, 16, 320, 320) -> (B, 32, 160, 160)
        self.p2.append(CSP(width[2], width[3], depth[0], csp[0], r=4)) #(B, 32, 160, 160) -> (B, 64, 160, 160)
        # p3/8
        self.p3.append(Conv(width[3], width[3], torch.nn.SiLU(), k=3, s=2, p=1)) #(B, 64, 160, 160) -> (B, 64, 80, 80)
        self.p3.append(CSP(width[3], width[4], depth[1], csp[0], r=4)) #(B, 64, 80, 80) -> (B, 128, 80, 80)
        # p4/16
        self.p4.append(Conv(width[4], width[4], torch.nn.SiLU(), k=3, s=2, p=1)) #(B, 128, 80, 80) -> (B, 128, 40, 40)
        self.p4.append(CSP(width[4], width[4], depth[2], csp[1], r=2)) #(B, 128, 40, 40) -> (B, 128, 40, 40)
        # p5/32
        self.p5.append(Conv(width[4], width[5], torch.nn.SiLU(), k=3, s=2, p=1)) #(B, 128, 40, 40) -> (B, 256, 20, 20)
        self.p5.append(CSP(width[5], width[5], depth[3], csp[1], r=2)) #(B, 256, 20, 20) -> (B, 256, 20, 20)
        self.p5.append(SPP(width[5], width[5])) #(B, 256, 20, 20) -> (B, 256, 20, 20)
        self.p5.append(PSA(width[5], depth[4])) #(B, 256, 20, 20) -> (B, 256, 20, 20)
    ...
    def forward(self, x):
        p1 = self.p1(x)   # (B, 16, 320, 320)
        p2 = self.p2(p1)  # (B, 64, 160, 160)
        p3 = self.p3(p2)  # (B, 128, 80, 80)
        p4 = self.p4(p3)  # (B, 128, 40, 40)
        p5 = self.p5(p4)  # (B, 256, 20, 20)
        return p3, p4, p5
...
class DarkFPN(torch.nn.Module):
    def __init__(self, width, depth, csp):
        ...
        # h1: (p5 upsample + p4) -> (B, 128, 40, 40)
        self.h1 = CSP(width[4] + width[5], width[4], depth[5], csp[0], r=2)
        # h2: (p4 upsample + p3) -> (B, 64, 80, 80)
        self.h2 = CSP(width[4] + width[4], width[3], depth[5], csp[0], r=2)

        # h3: downsample p3 -> (B, 64, 40, 40)
        self.h3 = Conv(width[3], width[3], torch.nn.SiLU(), k=3, s=2, p=1)
        # h4: (p3_down + p4) -> (B, 128, 40, 40)
        self.h4 = CSP(width[3] + width[4], width[4], depth[5], csp[0], r=2)

        # h5: downsample p4 -> (B, 128, 20, 20)
        self.h5 = Conv(width[4], width[4], torch.nn.SiLU(), k=3, s=2, p=1)
        # h6: (p4_down + p5) -> (B, 256, 20, 20)
        self.h6 = CSP(width[4] + width[5], width[5], depth[5], csp[1], r=2)

    def forward(self, x):
        p3, p4, p5 = x
        # top-down fusion   
        p4 = self.h1(torch.cat(tensors=[self.up(p5), p4], dim=1))  #(B, 128, 40, 40)
        p3 = self.h2(torch.cat(tensors=[self.up(p4), p3], dim=1))  #(B, 64, 80, 80)

        # bottom-up fusion
        p4 = self.h4(torch.cat(tensors=[self.h3(p3), p4], dim=1))  #(B, 128, 40, 40)
        p5 = self.h6(torch.cat(tensors=[self.h5(p4), p5], dim=1))  #(B, 256, 20, 20)
        return p3, p4, p5
